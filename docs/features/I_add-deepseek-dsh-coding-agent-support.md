...
updated_at: "2026-09-16T17:34:20.853Z"
---
...
- In:
    ...
    - Log format: the session header event (first line `type:"session"`) is retained as one `history_message` metadata row (`role=meta`, `disposition=meta`) carrying session id, `cwd` and `createdAt`; `user/message` and `assistant/message` events (`{type,seq,time,data}`) derive the conversational records
    ...
    - Unit tests: shim argv/registry, detector mock parse; importer source definition, split/mapper over fixture events (raw + decompressed), session-header metadata and corrupt-line handling
...
Feature: Add DeepSeek dsh coding agent support
  ...
  Scenario: R5 — zstd session log imports
    ...
    Then the header line is retained as one history_message metadata row (role=meta, disposition=meta) carrying session id, cwd and creation time, message events map to conversational rows with role, content, created_at (event time ms → ISO), model/provider when present, and source_record_id derived from message id, and the fixture still yields three imported records
...
| WBS | Task | Status |
| --- | ---- | ------ |
| 0066 | Add deepseek dsh shim to ts-ai-runner | done |
| 0067 | Add deepseek session source to ts-llm-jsonl-importer | done |
<!-- END AUTO-GENERATED -->
