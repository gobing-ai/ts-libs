...
updated_at: "2026-09-16T20:22:08.671Z"
feature_id: E
...
|-------------|--------|----------|
| R1 | MET | All current claims agree on the retained metadata row: feature I Scope `docs/features/I_add-deepseek-dsh-coding-agent-support.md:28` and AC R5 `:73`; task 0067 R2 `docs/tasks/0067_add-deepseek-session-source-to-ts-llm-jsonl-importer.md:28`, AC R5 `:49`, Design `:99`/`:102`, dated clarification `:85-89`; `CHANGELOG.md:34`; `dshSplit` docblock `packages/llm-jsonl-importer/src/mappers.ts:1450-1453`. The round-2 residual is closed: the 2026-09-11 Q&A bullet `docs/tasks/0067_add-deepseek-session-source-to-ts-llm-jsonl-importer.md:81` now ends with the inline superseded label "*(Superseded 2026-09-16 by the clarification below: ... retains the `type:"session"` header as one `meta` row rather than skipping it.)*". Fresh probe this stage: `dshSplit({type:'session',id:'sess-1',cwd:'/tmp/proj',createdAt:'2026-09-11T00:00:00Z'})` returns exactly 1 entry — `targetTable: history_message`, `role: meta`, `record_type: session`, `disposition: meta`, `ts`/`cwd` carried, `content_text: null`. `packages/llm-jsonl-importer/tests/deepseek-importer.test.ts:126` asserts `importedRecords` 3 and `:132-133` two `keep` rows; focused run 56 pass / 0 fail. |
| R2 | MET | Dated `**Amendment (2026-09-16, task 0068 C02)**` at `docs/00_ADR.md:358-362` supersedes only the R16 eager-materialization guarantee and leaves the 2026-08-12 addendum readable; frontmatter `docs/00_ADR.md:6`/`:9` bumped 1.0.0→1.1.0 with `updated_at: 2026-09-16`; historical task 0061 keeps its R2/R3 acceptance record with a dated supersession note at `docs/tasks/0061_fix-packages-review-session-bottlenecks-skill-path-drift-dua.md:119-124`. Fresh probe: `HISTORY_IMPORT_SCHEMA_SQL` CREATE targets are `history_import_checkpoint`, `history_import_ledger`, `history_message`, `history_tool_call`, `history_skill_call` (zero `history_etl_*`), and `applyHistoryImportSchema` on a fresh `:memory:` adapter leaves zero `history_etl_*` tables; `packages/llm-jsonl-importer/tests/jsonl-importer-dao.test.ts:315-329` asserts the same. No source or test edit. |
| R3 | MET | E roster `docs/features/E_ts-llm-jsonl-importer.md:31-33` = 0063 `done`, 0064 `done`, 0068 `wip`, equal to fresh `spur task list --feature E --json`; I roster `docs/features/I_add-deepseek-dsh-coding-agent-support.md:105-106` = 0066/0067 `done`, equal to fresh `spur task list --feature I --json`. `docs/05_FEATURES.md:35` adds feature I with its real lifecycle status (`active` maps to partial, the same mapping as E) plus satellite pointer; feature I frontmatter `status: active` is unchanged vs HEAD, so no lifecycle advanced; `docs/features/INDEX.md` is byte-identical to HEAD. |
| R4 | MET | `packages/runtime/README.md:184` annotates the canonical `FileSystem` (`createNodeFileSystem`); `packages/runtime/README.md:438-440` states that `getFs()`/`setFileSystem()`, `SyncFileSystem` and the old filesystem classes "were removed in ADR-019" and points at `createNodeFileSystem()`/`createCfFileSystem()`/`ctx.require('fileSystem')`. `docs/00_ADR.md:283-294` (ADR-019) confirms the removal; `packages/runtime/src/index.ts:5-7` exports only the canonical `FileSystem` type plus the two factories; a fresh grep over `packages/runtime/src` finds no `getFs`/`setFileSystem`/`SyncFileSystem` and no removed export was restored. Retained deprecations untouched (`packages/runtime/README.md:189`, `:641`). |
| R5 | MET | `packages/rule-engine/README.md:39` describes capability kind, the authored relative path, the absolute declaring `baseDir` and `sourceName` with pre-resolution validation, matching `packages/rule-engine/src/config/extensions.ts:19-28` field for field. A grep finds no `absPath`/`presetName` in the README; the loader is unmodified; `packages/rule-engine/tests/config/extensions.test.ts` is green in the focused 56 pass / 0 fail run. |
| R6 | MET | `packages/llm-jsonl-importer/README.md:64-80` bounds `O(line)` to the line-reading component, names both full-text-buffering paths (whole-file `zstd` decompression; `readFile` plus split when `readFileStream` is absent) and excludes discovery/checkpoint/ledger memory. Verified against the three real read paths in `packages/llm-jsonl-importer/src/importer.ts:732-750`: `.jsonl.zstd` decompresses whole first, `readFileStream` is consulted only for uncompressed files, and the `readFile` plus split fallback follows; `packages/llm-jsonl-importer/src/zstd.ts:10-11` documents the buffered ceiling. `importer.ts` and `zstd.ts` are unmodified (baseline fingerprints matched) — no streaming refactor. |
| R7 | MET | `packages/llm-jsonl-importer/README.md:22` roster is claude, codex, gemini, pi, opencode, antigravity, openclaw, omp, grok, agy, deepseek. Fresh probe of `Object.keys(SOURCE_DEFINITIONS)` from `packages/llm-jsonl-importer/src/sources.ts:158` prints agy, antigravity, claude, codex, deepseek, gemini, grok, omp, openclaw, opencode, pi — set-equal, 11 keys, each exactly once; agy and antigravity kept as distinct identities; the registry file is unmodified (baseline fingerprint matched). |
| R8 | MET | All four in-scope occurrences are Spur-qualified: `docs/03_ARCHITECTURE.md:72` (Spur task 0734), `:114` (Spur feature A21 / Spur ADR-112), `:140` (Spur A21) and `packages/llm-jsonl-importer/README.md:84`. Upstream verified before linking: `git -C ../spur-new cat-file -e origin/main:<path>` EXISTS for `docs/tasks4/0734_configurable-scheduler-jobs-interval-real-cron-in-ts-libs-ad.md`, `docs/features/A21_reusable-execution-deadlines-and-unlimited-jobs.md` and `docs/00_ADR.md`; `origin` = https://github.com/gobing-ai/spur.git; the ADR-112 heading resolves at `origin/main:docs/00_ADR.md:1621`. No local ADR-112 stub and no sibling edit. |
| R9 | MET | All 24 Design baseline fingerprints (`docs/tasks/0068_resolve-package-contract-and-documentation-conflicts-c01-c08.md:525-553`) recomputed this stage against both `git show HEAD:<file>` and `git show 3d1d074:<file>` → 24/24 exact, 0 mismatches. Ordering: the authorization Q&A entry `docs/tasks/0068_resolve-package-contract-and-documentation-conflicts-c01-c08.md:118` is stamped 2026-09-16T17:32:37Z, before every dependent corpus edit (task 0061 `updated_at` 17:33:10Z, feature I 17:34:20Z, task 0067 alignment 19:43:40Z). Numbered-doc frontmatter version and date bumped on all three affected docs: `docs/00_ADR.md:6`/`:9` 1.1.0, `docs/03_ARCHITECTURE.md:6`/`:9` 1.2.0, `docs/05_FEATURES.md:6`/`:9` 1.3.0, each `updated_at: 2026-09-16`; ADR-023, task 0061 and task 0067 history remain readable; `docs/features/INDEX.md` byte-identical. |
| R10 | MET | Fresh this stage: `bun run spur-check` exit 0 (biome 426 files clean, 8/8 typechecks exit 0, 49 pre-check rules pass, 2250 pass / 0 fail, 5949 expect calls across 192 files, 2 post-check rules pass) and `bun run build` 8/8 packages exit 0. Driver gate log `.spur/run/0068-test-gate.log` with `.spur/run/0068-test-gate.status` = PASS, re-run at 12:47 after the 12:43 R1 repair. `spur task check 0068`/`0067`/`0061` PASS; `spur feature check E`/`I` PASS (feature I carries pre-existing warnings naming task 0066). `git status --porcelain` shows exactly 13 modified files, every one inside a Design permitted repair surface (`docs/tasks/0068_resolve-package-contract-and-documentation-conflicts-c01-c08.md:577-588`), with 0 untracked; no test, dependency, export, schema, `.github/workflows`, publication or sibling change. |

| Acceptance Criteria | Status | Evidence Type | Evidence |
|---------------------|--------|---------------|----------|
| Scenario: R1 — DeepSeek header contract matches its approved behavior | MET | test | Claims agree on the retained metadata row across `docs/features/I_add-deepseek-dsh-coding-agent-support.md:28`/`:73`, `docs/tasks/0067_add-deepseek-session-source-to-ts-llm-jsonl-importer.md:28`/`:49`/`:99`/`:81`, `CHANGELOG.md:34` and the `dshSplit` docblock `packages/llm-jsonl-importer/src/mappers.ts:1450-1453`; the fresh probe yields one meta row for a session header and `packages/llm-jsonl-importer/tests/deepseek-importer.test.ts:126`/`:132-133` prove `importedRecords` stays 3 with 2 keep rows (focused run 56 pass / 0 fail). |
| Scenario: R2 — ETL creation authority is reconciled before projections | MET | test | Dated amendment recorded before projections (`docs/00_ADR.md:358-362`), original ADR-023 addendum and task 0061 history readable (`docs/tasks/0061_fix-packages-review-session-bottlenecks-skill-path-drift-dua.md:119-124`), and the fresh `:memory:` probe returns zero `history_etl_*` tables, matching `packages/llm-jsonl-importer/tests/jsonl-importer-dao.test.ts:315-329`. |
| Scenario: R3 — Feature rosters and numbered summary reflect current corpus | MET | command | `docs/features/E_ts-llm-jsonl-importer.md:31-33` (0063 done, 0064 done, 0068 wip) equals `spur task list --feature E --json`; `docs/features/I_add-deepseek-dsh-coding-agent-support.md:105-106` (0066/0067 done) equals `spur task list --feature I --json`; `docs/05_FEATURES.md:35` adds feature I with its real lifecycle status and satellite pointer; no feature lifecycle advanced and `docs/features/INDEX.md` is byte-identical to HEAD. |
| Scenario: R4 — Runtime documentation no longer advertises removed filesystem APIs | MET | command | `packages/runtime/README.md:184` uses the canonical `FileSystem` contract and `:438-440` describes `getFs`/`setFileSystem`/`SyncFileSystem` and the old classes as removed by ADR-019 (`docs/00_ADR.md:283-294`); a grep over `packages/runtime/src` and `packages/runtime/src/index.ts:5-7` shows no removed export and none was restored. |
| Scenario: R5 — ExtensionRef usage documents the authored-path contract | MET | command | `packages/rule-engine/README.md:39` describes kind, authored relative path, absolute `baseDir` and `sourceName` with pre-resolution validation, matching `packages/rule-engine/src/config/extensions.ts:19-28`; no absolute authored path is required or accepted and the loader is unchanged. |
| Scenario: R6 — Streaming documentation exposes both buffering exceptions | MET | command | `packages/llm-jsonl-importer/README.md:64-80` distinguishes uncompressed line streaming from whole-file zstd decompression and the `readFile` fallback and bounds `O(line)` to the line reader, matching `packages/llm-jsonl-importer/src/importer.ts:732-750` and `packages/llm-jsonl-importer/src/zstd.ts:10-11`; no new streaming implementation and no end-to-end `O(line)` promise. |
| Scenario: R7 — Built-in source roster is complete | MET | command | `packages/llm-jsonl-importer/README.md:22` roster equals the live `Object.keys(SOURCE_DEFINITIONS)` from `packages/llm-jsonl-importer/src/sources.ts:158` (11 keys, each once, agy and antigravity distinct); no registry key, alias or ordering changed. |
| Scenario: R8 — Cross-repository authority provenance is explicit | MET | command | `docs/03_ARCHITECTURE.md:72`/`:114`/`:140` and `packages/llm-jsonl-importer/README.md:84` each name Spur; the three linked upstream records exist on `origin/main` of gobing-ai/spur and the ADR-112 heading resolves at `origin/main:docs/00_ADR.md:1621`; no local placeholder record and no sibling edit. |
| Scenario: R9 — Fresh evidence and owner routing preserve authority and history | MET | command | 24/24 baseline fingerprints matched both `HEAD` and the audit baseline `3d1d074`, the 2026-09-16T17:32:37Z authorization precedes every dependent edit, numbered-doc version/date bumped on `docs/00_ADR.md:6`/`:9`, `docs/03_ARCHITECTURE.md:6`/`:9`, `docs/05_FEATURES.md:6`/`:9`, and historical decisions remain readable. |
| Scenario: R10 — Fresh verification and the bounded diff establish completion | MET | command | Fresh `bun run spur-check` (2250 pass / 0 fail, 49 pre-check plus 2 post-check rules) and fresh `bun run build` (8/8 packages exit 0) with no suppression; `spur task check 0068` and `spur feature check E`/`I` pass; all 13 changed files map to the C01-C08 allowlist with no test, dependency, workflow, publication or sibling change. |
