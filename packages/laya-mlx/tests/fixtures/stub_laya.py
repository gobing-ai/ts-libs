    ...
    def predict(self, state, questions):
        ...
        for start in range(0, len(question_ids), self.batch_size):
            ...
            for qid in question_ids[start : start + self.batch_size]:
                ...
                # Mirror the runtime's per-kind criteria validation (agent.py
                # `_to_internal`) so the protocol lane catches wire-shape drift
                # instead of accepting shapes the real Agent would reject.
