...
import {
    ownProcessGroupLifecycle,
    type ProcessGroupOwnership,
    resolveDeadline,
    resolveKillGraceMs,
} from './process-group';
...
/**
 * Sync process execution surface shared by the sync executors (task 0087 R7).
 * Not deprecated — the deprecated part is the Bun-specific default, not the shape.
 */
...
/**
 * Sync executor on `node:child_process.spawnSync` — the non-deprecated default
 * for sync callers (task 0087 R7). Contract-compatible with
 * {@link BunSyncProcessExecutor} for success and non-zero exits. One deliberate
 * divergence: a spawn failure (e.g. missing binary) maps to exitCode 1 instead of
 * throwing, so `getGitContextSync`-style probes see a plain non-zero exit.
 */
export class NodeSyncProcessExecutor implements SyncProcessExecutor {
    runSync(options: Omit<ProcessOptions, 'timeout'>): ProcessResult {
        const args = options.args ?? [];
        const startedAt = Date.now();
        ...
        if (options.rejectOnError === true && exitCode !== 0) {
            throw new Error(
                `${options.command} ${args.join(' ')} failed with exit code ${exitCode}: ${stripFinalNewline(
                    asString(result.stderr),
                )}`,
            );
        }
